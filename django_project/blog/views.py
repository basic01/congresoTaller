...
from .models import Participante, Detalle, Programa, Taller, Inscripcion
from .forms import ParticipanteForm, InscripcionForm, AdminGeneralForm, AdminTalleresForm, AdminProgramaForm, AdminInscripcionesForm
...
def registro(request):
	if request.method == 'POST':
		...
		form = ParticipanteForm()
	return render(request, 'blog/registro.html', {'title':'Registro', 'form': form})

def exito(request, tipoExito):
	if tipoExito != 'r' and tipoExito != 'i':
		return redirect('taller:tallerInicio')	
	return render(request, 'blog/exito.html',{'title':'Success','tipoExito': tipoExito})	
...
def talleres(request):
	...
	return render(request, 'blog/talleres.html', detalles)
...
def programa(request):
	...
	return render(request, 'blog/programa.html', detalles )
...
def warning(request, i, t, bandera):
	...
	return render(request, 'blog/warning.html',detalles)	
...
def inscripcion(request, idTaller):
	...
	return render(request, 'blog/inscripcion.html', detalles)
...
def editarGeneral(request, idElemento):
	elemento = Detalle.objects.get(id = idElemento)
	if request.method == 'GET':
		form = AdminGeneralForm(instance=elemento)
	else:
		form = AdminGeneralForm(request.POST, instance=elemento)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorGeneral')
	return render(request, 'admin/generalForm.html', {'form':form})

class ViewParticipantes(ListView):
	model = Participante
	queryset = Participante.objects.order_by('apellidoPaterno','apellidoMaterno','nombre')
	template_name = 'admin/participantes.html'
	detalles = {
...
def participantesMondragon(request):
	...
	return render(request, 'admin/participantesMondragon.html', contexto)	
...
def nuevoParticipante(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			form.save()
		return redirect('taller:administradorParticipantes')
	else:
		form = ParticipanteForm()
	return render(request, 'admin/participantesForm.html', {'form':form})

def eliminarParticipante(request, idElemento):
	elemento = Participante.objects.get(id=idElemento)
	if(request.method == 'GET'):
		elemento.delete()
		return redirect('taller:administradorParticipantes')

def editarParticipante(request, idElemento):
	elemento = Participante.objects.get(id = idElemento)
	if request.method == 'GET':
		form = ParticipanteForm(instance=elemento)
	else:
		form = ParticipanteForm(request.POST, instance=elemento)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorParticipantes')
	return render(request, 'admin/participantesForm.html', {'form':form})
...
class ViewPrograma(ListView):
	...
	template_name = 'admin/programa.html'
	detalles = {
...
def nuevoPrograma(request):
	if request.method == 'POST':
		form = AdminProgramaForm(request.POST)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorPrograma')
		messages.success(request, 'Exito')
	else:
		form = AdminProgramaForm()
	return render(request, 'admin/participantesForm.html', {'form':form})

def eliminarPrograma(request, idElemento):
	elemento = Programa.objects.get(id = idElemento)
	if(request.method == 'GET'):
		elemento.delete()
		return redirect('taller:administradorPrograma')


def editarPrograma(request, idElemento):
	elemento = Programa.objects.get(id = idElemento)
	if request.method == 'GET':
		form = AdminProgramaForm(instance=elemento)
	else:
		form = AdminProgramaForm(request.POST, instance=elemento)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorPrograma')
	return render(request, 'admin/programaForm.html', {'form':form})

class ViewTalleres(ListView):
	model = Taller
	queryset = Taller.objects.order_by('dia','horario','nombre')
	template_name = 'admin/talleres.html'
	detalles = {
...
def talleresAdmin(request):
	...
	return render(request, 'admin/talleres.html', detalles)


def nuevoTaller(request):
	if request.method == 'POST':
		form = AdminTalleresForm(request.POST)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorTalleres')
		messages.success(request, 'Exito')
	else:
		form = AdminTalleresForm()
	return render(request, 'admin/talleresForm.html', {'form':form})

def eliminarTaller(request, idElemento):
	elemento = Taller.objects.get(id = idElemento)
	if(request.method == 'GET'):
		elemento.delete()
		return redirect('taller:administradorTalleres')

def editarTaller(request, idElemento):
	elemento = Taller.objects.get(id = idElemento)
	if request.method == 'GET':
		form = AdminTalleresForm(instance=elemento)
	else:
		form = AdminTalleresForm(request.POST, instance=elemento)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorTalleres')
	return render(request, 'admin/talleresForm.html', {'form':form})
...
class ViewInscripciones(ListView):
	...
	template_name = 'admin/inscripciones.html'
	detalles = {
...
def nuevoInscripcion(request):
	if request.method == 'POST':
		form = AdminInscripcionesForm(request.POST)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorInscripciones')
		messages.success(request, 'Exito')
	else:
		form = AdminInscripcionesForm()
	return render(request, 'admin/inscripcionesForm.html', {'form':form})

def eliminarInscripcion(request, idElemento):
	elemento = Inscripcion.objects.get(id = idElemento)
	if(request.method == 'GET'):
		elemento.delete()
		return redirect('taller:administradorInscripciones')

def editarInscripcion(request, idElemento):
	elemento = Inscripcion.objects.get(id = idElemento)
	if request.method == 'GET':
		form = AdminInscripcionesForm(instance=elemento)
	else:
		form = AdminInscripcionesForm(request.POST, instance=elemento)
		if form.is_valid():
			form.save()
		return redirect('taller:administradorInscripciones')
	return render(request, 'admin/inscripcionesForm.html', {'form':form})